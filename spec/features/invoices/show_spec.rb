...
RSpec.describe 'invoices show' do
  ...
  it "shows the total and discounted revenue for this merchant" do
    ...
    ii_3 = InvoiceItem.create!(invoice_id: invoice.id, item_id: item_3.id, quantity: 10, unit_price: 10, status: 1)
    ii_4 = InvoiceItem.create!(invoice_id: invoice.id, item_id: item_4.id, quantity: 20, unit_price: 10, status: 1)
    ...
    within "#discounts" do
      expect(page).to have_content("Total Merchant Revenue: $300.00")
      expect(page).to have_content("Merchant Bulk Discount: $125.00")
      expect(page).to have_content("Discounted Merchant Revenue: $175.00")
    end
  end

  it "has link to applied discount (if any) for each invoice item" do
    merchant_1 = Merchant.create!(name: 'Hair Care')
    merchant_2 = Merchant.create!(name: 'Eye Care')
    item_1 = Item.create!(name: "Shampoo", description: "This washes your hair", unit_price: 10, merchant_id: merchant_1.id, status: 1)
    item_2 = Item.create!(name: "Butterfly Clip", description: "This holds up your hair but in a clip", unit_price: 5, merchant_id: merchant_1.id)
    ...
    within ".table" do
      within "#discount-0" do
        expect(page).to have_link("25%")
        expect(page).to_not have_link("50%")
      end
      
      within "#discount-2" do
        expect(page).to_not have_link("25%")
        expect(page).to_not have_link("50%")
      end
      
      within "#discount-3" do
        expect(page).to_not have_link("25%")
        expect(page).to_not have_link("50%")
      end
